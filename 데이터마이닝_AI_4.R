...
#KNN은 instance-based learning으로 모델을 생성하지 않고 데이터를 분류하거나 예측할 때 사용하는 방법
...
md2<-knn(train=iris_train,test =iris_test, cl=iris_train_label,k=3 )

md3<-knn(train=iris_train,test =iris_test, cl=iris_train_label,k=7 )

md4<-knn(train=iris_train,test =iris_test, cl=iris_train_label,k=10 )
...
CrossTable(iris_test_label,md4)


CrossTable(iris_test_label,md3)
...
#회귀 분석은 데이터를 정확하게 맞출 수 있는 선을 찾는 반면에 , SVM은 최적의 분류 직선을 찾는다. 
#최적화된 선을 찾기 위해서 A그룹과 B그룹의 가장 바깥 쪽에 있는 데이터를 찾는다.
# 즉 A그룹과 B그룹의 바깥 쪽에 있는 데이터 간의 거리가 가장 멀리 떨어진 직선을 찾는다.
# 각 직선이 평행하게 되어 있으며, 거리가 최대가 되고 마진이 떨어지게 된다. 
