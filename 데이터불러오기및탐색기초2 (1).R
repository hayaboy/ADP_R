...
data.frame(sex=c("M","F",NA,"M","F"),score=c(4,5,6,5,NA))
sex_score<-data.frame(sex=c("M","F",NA,"M","F"),score=c(4,5,6,5,NA))

str(sex_score)
sum(sex_score$score,na.rm = TRUE)

is.na(sex_score$score)

library(dplyr)
sex_score %>% filter(!is.na(sex))


sex_score %>% filter(!is.na(score))
colnames(sex_score)
...
mean_sex_score<-mean(sex_score$score,na.rm=T)
mean_sex_score




sex_score$score<-ifelse(is.na(sex_score$score),mean_sex_score, sex_score$score)




outlier_ex<-c(1,500,550,400,480,600,590)
boxplot(outlier_ex)
...
me<-ifelse(is.na(outlier_ex),median(outlier_ex),outlier_ex)
ifelse(outlier_ex>=500 & outlier_ex<=600,)
