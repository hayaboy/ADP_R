...
#MASS 패키지의 Cars93 데이터에서 엔진크기(EngineSize), RPM, 무게(Weight)를 독립변수로 설정하고 
#자동차 가격(Price)을 종속 변수로 설정하여 다변량 회귀 분석 수행하고, 그 결과를 해석
...
cor(Cars93[ , c(5)] , Cars93[ , c(-1, -2, -3, -5, -9, -10, -11, -16, -26,-27)])
...
CarPrice_lm<- lm( Price   ~  EngineSize + RPM + Weight , data = Cars93 )
...
#2회귀 계수 
#상수항과 세 변수에 대한 회귀 계수는 모두 통계적으로 유의하다.


#3모형의 설명력

#Adjusted R-squared:  0.5467 (수정된 결정계수는 0.5467이므로 회귀모형이 
#전체 데이터의 약 54.67%를 설명할 수 있다.
#조금 낮게 나타났기 때문에 해당 회귀식이 데이터를 적절하게 설명하고 있다고는 할 수 없다.

#4 모형의 통계적 유의성

#F-통계량은 37.98이며 유의확률 (p-value: 6.746e-16)이므로, 유의 수준 0.05 하에서
#추정된 회귀 모형이 통계적으로 매우 유의함을 알 수 있다.

#최종 정리 : 자동차의 가격을 EngineSize, RPM,  Weight로 추정할 수 있다.



#최적의 회귀 방정식 선택(변수 선택법)

#가. 단계적 변수 선택(Stepwise)  

   #1)전진 선택법(forward)
#        : 절편만 있는 상수 모형에서 시작하여 중요하다고 생각되는 설명변수부터 차례로 모형에 추가 

   #2)후진 선택법(backward)
#       : 모든 독립변수를 포함한 모형에서 출발하여 종속변수에 가장 적은 영향을 주는 변수부터 하나씩 제거하면서 더 이상 변수가 없을 때의 
         #모형을 선택

   #3)단계적 방법 (Stepwise)
#      : 전진 선택법에 의해 변수를 추가하면서 새롭게 추가된 변수에 의해 기존 변수의 중요도가 약화되면 해당 변수를 제거한다.
#        이처럼 단계별로 변수의 추가 또는 제거를 수행하면서 기준 통계치를 가장 많이 개선시킬 때까지 반복. 반대로도 가능
         


#나. 벌점화된 선택 기준

#    : 모형의 복잡도에 따라 벌점을 주는 방식, AIC, BIC 방법이 주로 사용, 일반적으로 AIC 활용


#AIC(Akaike Information Criterion) : 낮은게 좋은 모형
...
summary(lm_a)
#회귀모형은 통계적으로 유의하나 입력 변수들의 통계적 유의성을 검토해보니 Width, Length   변수의
#회귀 계수에 대한 유의확률이 0.05보다 큰 것을 확인

#적절한 모형을 선정하기 위해 Width 변수를 제외(유의 확률이 가장 높은 것 우선 제거) 하고 다시 회귀모형 lm_b를 만들어 보자.
...
#R에서 변수 선택을 자동적으로 수행하는 함수

?step()

step(object, scope, direction, k)

#objecct는 변수 선택을 진행할 회귀 모형
#scope는 변수 선택 과정에서 사용되는 모형의 범위( list 내부에  모형의 상한은 upper, 하한은 lower)
#direcion은 변수 선택 방법("both", "backward", "forward") 
...
#y= -8690.818    +  184.184     + 241.345     -202.160       -8.445




#변수 선택 결과를 이용해 회귀 모형 생성
